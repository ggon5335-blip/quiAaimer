...
// SOCKET EVENTS
socket.on('room-created',d=>{S.roomCode=d.code;S.isHost=true;S.players=d.players;document.getElementById('lobby-code').textContent=d.code;renderLobby(d.players,S.myId);initConfig(d.categories,d.modes,{rounds:10,voteTimer:12,guessTimer:15,categories:Object.keys(d.categories),mode:'classic'},true);generateQR(d.code);document.getElementById('chat-widget').style.display='block';goToScreen('screen-lobby');playSound('success')});
socket.on('room-joined',d=>{S.roomCode=d.code;S.isHost=false;S.players=d.players;document.getElementById('lobby-code').textContent=d.code;renderLobby(d.players,d.hostId);initConfig(d.categories,d.modes,d.config,false);generateQR(d.code);document.getElementById('chat-widget').style.display='block';goToScreen('screen-lobby');playSound('success')});
